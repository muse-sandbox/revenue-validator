# Synthetic answer — computed claim under an ordinal heading (selftest)
...
## 6. What this experiment cannot show

- [computed] 64% × (1 − 25%) ≈ 48% of the test arm can actually
  reach the widget, so a true +30% on those users arrives in the overall
  metric as 48% × 30% ≈ 14%, against the 3 pp this design resolves. A
  null result here would not mean the widget did nothing.
