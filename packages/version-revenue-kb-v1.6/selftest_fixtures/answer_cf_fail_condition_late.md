...
- **[stop]** [topic: guardrail-margin-synthetic] We spend the slot and come back with a guardrail read that cannot clear this launch. *Mechanism:* [computed] the guardrail resolves 4.2 pp while the margin the team approved is 1.5 pp, so 4.2 / 1.5 = 2.8 times that margin. *If:* nothing changes and the guardrail keeps the approved margin. *Consequence:* a non-significant guardrail stays compatible with almost three times the loss the team said it would accept, so the guardrail condition cannot be declared met on any outcome. *Price:* decision impossible — the question this design was built to settle stays open whatever the numbers say. *Fix:* re-size the guardrail on the approved 1.5 pp margin before launch.
...
- **[mechanic]** [topic: passive-toast-synthetic-surface] *If:* the invented widget runs as a passive toast instead of an interrupt, keeping the 64% branch reach. *Grounds:* [interpretation] T9-01 (L1 card below) read its own lift as coming from the passive form rather than from the offer. *Then:* the invented metric moves up on the exposed users.
