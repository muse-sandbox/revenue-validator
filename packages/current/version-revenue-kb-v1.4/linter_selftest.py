#!/usr/bin/env python3
"""Deterministic selftest for linter.py (Revenue Validator V1.4, FLOW-632).
...
derive from any real or holdout experiment case. The V1.1/V1.2/V1.3 fixture
answers and fixture_kb.md / fixture_kb_gc.md / fixture_patterns.md are
inherited byte-identically. Because V1.4 makes the
`## What this experiment cannot show` section mandatory, those inherited
answers no longer pass as written: the `v14_delta_*` blocks below assert that
the ONLY code they acquire under the V1.4 linter is
`E_MISSING_COMPUTED_SLOT` (on top of the V1.3 delta for the older ones) and
that their warnings are unchanged — that is the regression proof for this
version. The `cc_*` block tests §2.8 itself, including the pair of runs that
proves the linter tells invention and transfer apart on the SAME answer,
using only the presence of the number in the knowledge base. Run:
...
def main():
    ...
    # V1.4 adds a second mandatory section, so the delta an inherited answer
    # acquires is now two codes: the V1.3 one and the V1.4 one. Nothing else
    # may change — that is the V1.1/V1.2/V1.3 regression proof for V1.4.
    V13_DELTA = [linter.E_MISSING_PRODUCT_PROPOSALS]
    V14_DELTA = [linter.E_MISSING_COMPUTED_SLOT]

    def delta_check(prefix, cases, kb=FIXTURE_KB, delta=None):
        delta = V13_DELTA + V14_DELTA if delta is None else delta
        for entry in cases:
            if len(entry) == 5:
    ...
    delta_check("v14_delta_", cli_cases)
    ...
    delta_check("v14_delta_", gc_cases, kb=FIXTURE_KB_GC)

    # The inherited fixtures must acquire the same delta whether or not the KB
    # declares generalization classes.
    delta_check("v14_delta_under_gc_kb_", cli_cases, kb=FIXTURE_KB_GC)

    # --- V1.3 CLI tests: product proposals (KB §2.7) ----------------------
    # Under V1.4 these acquire exactly one new code, E_MISSING_COMPUTED_SLOT,
    # because none of them carries the new mandatory section. Their warnings
    # must be unchanged: a V1.3 fixture emits no [computed] statement, so the
    # §2.8 machinery has nothing to say about it beyond the missing slot.
    pp_cases = [
    ...
    for name, answer, no_kb, _v13_exit, v13_codes, want_warnings in pp_cases:
        ...
        if report is None:
            check("v14_delta_" + name, False, "linter stdout was not JSON")
            continue
        got_codes = error_codes(report)
        got_warnings = warning_codes(report)
        want_codes = sorted(set(v13_codes) | set(V14_DELTA))
        ok = (code == 1 and got_codes == want_codes
              and got_warnings == sorted(want_warnings))
        check("v14_delta_" + name, ok,
    ...
    # Every fixture below carries the V1.3 obligations, so the only codes it
    # can produce are §2.8 ones. `--card` is the synthetic experiment card.
    cc_cases = [
    ...
    for name, answer, no_kb, card, want_exit, want_codes, want_warnings in cc_cases:
    ...
    check("cc_fail_kb_transfer_is_diagnosed_as_transfer",
          ...
          and error_codes(report_kb) == [linter.E_COMPUTED_NUMBER_FROM_KB],
    ...
    check("cc_same_number_absent_from_kb_is_fabricated",
          ...
          and error_codes(report_fab) == [linter.E_COMPUTED_NUMBER_FABRICATED],
    ...
    check("cc_missing_card_warns_instead_of_passing_silently",
          code_nocard == 0 and report_nocard is not None
          and warning_codes(report_nocard) == [linter.W_CARD_NOT_SUPPLIED]
          and error_codes(report_nocard) == [],
          "codes=%s warnings=%s"
    ...
    check("pp_warning_does_not_fail_the_answer",
          ...
          and linter.E_MISSING_COMPUTED_SLOT in error_codes(warn_report)
