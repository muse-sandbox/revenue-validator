#!/usr/bin/env python3
"""Deterministic selftest for linter.py (Revenue Validator V1.6, FLOW-641).
...
V1.6 adds §2.10 (conditional form), §2.11 (one idea, one place) and §2.12
(the generic-advice screen). Those rules fire per bullet, so an inherited
fixture's delta depends on how many bullets it has; the `v16_delta` helper
therefore asserts the property rather than a fixed code list — no inherited
code or warning disappears, and everything gained is a V1.6 code. The `v16_*`
block tests the new rules themselves on 16 synthetic fixtures, one per code.
Run:
...
def main():
    ...
    # The codes and warnings V1.6 is allowed to add to an inherited fixture.
    # Anything outside these sets means V1.6 disturbed an older rule.
    V16_CODES = frozenset((
        linter.E_FINDING_NO_CONDITION, linter.E_GROUNDS_UNLABELLED,
        linter.E_PROPOSAL_NO_CONDITIONAL_FORM,
        linter.E_CONSEQUENT_NO_DIRECTION, linter.E_TOPIC_MISSING,
        linter.E_TOPIC_MALFORMED, linter.E_DUPLICATE_TOPIC,
        linter.E_PROPOSAL_UNANCHORED, linter.E_GENERIC_UI_ADVICE,
    ))
    V16_WARNINGS = frozenset((
        linter.W_TOPIC_RESTATED_IN_APPENDIX, linter.W_FIX_UNANCHORED,
    ))

    def v16_delta(name, code, report, prior_codes, prior_warnings=None):
        """Assert the V1.6 delta property for one inherited fixture."""
        if report is None:
            check(name, False, "linter stdout was not JSON")
            return
        got = set(error_codes(report))
        want = set(prior_codes)
        lost = sorted(want - got)
        gained = sorted(got - want)
        stray = sorted(set(gained) - V16_CODES)
        detail = ("lost=%s stray=%s gained=%s exit=%d"
                  % (lost, stray, gained, code))
        ok = not lost and not stray and code == (1 if got else 0)
        if prior_warnings is not None:
            got_w = set(warning_codes(report))
            want_w = set(prior_warnings)
            lost_w = sorted(want_w - got_w)
            stray_w = sorted((got_w - want_w) - V16_WARNINGS)
            detail += " lost_warnings=%s stray_warnings=%s" % (lost_w, stray_w)
            ok = ok and not lost_w and not stray_w
    ...
    def delta_check(prefix, cases, kb=FIXTURE_KB, delta=None):
        ...
        for entry in cases:
            ...
            if report is None:
                ...
                continue
            v16_delta(prefix + name, code, report,
                      set(v12_codes) | set(delta))
    ...
    for name, answer, no_kb, _v13_exit, v13_codes, want_warnings in pp_cases:
        ...
        if report is None:
            ...
            continue
        v16_delta("v16_delta_" + name, code, report,
                  set(v13_codes) | set(V15_DELTA), want_warnings)
    ...
    for name, answer, no_kb, card, _v14_exit, v14_codes, want_warnings in cc_cases:
        ...
        if report is None:
            ...
            continue
        v16_delta("v16_delta_" + name, code, report,
                  (set(v14_codes) - V15_RETIRED) | set(V15_DELTA),
                  want_warnings)
    ...
    v16_delta("cc_fail_kb_transfer_is_diagnosed_as_transfer", code_kb,
              report_kb, {linter.E_COMPUTED_NUMBER_FROM_KB} | set(V15_DELTA))
    check("cc_kb_transfer_carries_no_invention_code",
          report_kb is not None
          and linter.E_COMPUTED_NUMBER_FABRICATED not in error_codes(report_kb),
    ...
    check("cc_same_number_absent_from_kb_is_fabricated",
          ...
          and linter.E_COMPUTED_NUMBER_FABRICATED in error_codes(report_fab)
          and linter.E_COMPUTED_NUMBER_FROM_KB not in error_codes(report_fab),
    ...
    for name, answer, no_kb, card, want_exit, want_codes, want_warnings in fd_cases:
        ...
        if report is None:
            check(name, False, "linter stdout was not JSON")
            continue
        v16_delta("v16_delta_" + name, code, report, set(want_codes),
                  want_warnings)
    ...
    def pp_all_codes(answer, no_kb=False, cards=()):
    ...
    def pp_codes(answer, no_kb=False, cards=()):
        """The §2.7 grounding codes alone — V1.6's form codes filtered out."""
        return [c for c in pp_all_codes(answer, no_kb, cards)
                if c not in linter.FORM_CODES]
    ...
    check("v16_unit_pp_bare_bullet_also_fails_the_form_rules",
          linter.E_PROPOSAL_NO_CONDITIONAL_FORM
          in pp_all_codes(pattern_only)
          and linter.E_PROPOSAL_NO_CONDITIONAL_FORM
          not in pp_codes(pattern_only),
          "a one-line proposal is grounded under §2.7 and formless under "
          "§2.10; the two verdicts are reported separately")
    ...
    # --- V1.6 CLI tests: conditional form, topics, generic screen ---------
    # KB §2.10 / §2.11 / §2.12. Every fixture is a complete V1.6 answer
    # generated from ONE compliant base by a single named edit, so each case
    # differs from the passing one in exactly the thing it tests. All content
    # is synthetic: the invented card, the invented widget, T9-01, P-90.
    v16_cases = [
        ("v16_cf_pass", "answer_cf_pass.md", False, 0, [], []),
        ("v16_cf_nokb_pass", "answer_cf_nokb_pass.md", True, 0, [], []),
        ("v16_cf_fail_no_condition", "answer_cf_fail_no_condition.md", False,
         1, [linter.E_FINDING_NO_CONDITION], []),
        ("v16_cf_fail_condition_after_mechanism",
         "answer_cf_fail_condition_late.md", False, 1,
         [linter.E_FINDING_NO_CONDITION], []),
        ("v16_cf_fail_grounds_unlabelled",
         "answer_cf_fail_grounds_unlabelled.md", False, 1,
         [linter.E_GROUNDS_UNLABELLED], []),
        ("v16_cf_fail_proposal_not_conditional",
         "answer_cf_fail_proposal_form.md", False, 1,
         [linter.E_PROPOSAL_NO_CONDITIONAL_FORM], []),
        ("v16_cf_fail_consequent_without_direction",
         "answer_cf_fail_no_direction.md", False, 1,
         [linter.E_CONSEQUENT_NO_DIRECTION], []),
        ("v16_uq_fail_topic_missing", "answer_uq_fail_topic_missing.md",
         False, 1, [linter.E_TOPIC_MISSING], []),
        ("v16_uq_fail_topic_malformed", "answer_uq_fail_topic_malformed.md",
         False, 1, [linter.E_TOPIC_MALFORMED], []),
        ("v16_uq_fail_duplicate_topic", "answer_uq_fail_duplicate_topic.md",
         False, 1, [linter.E_DUPLICATE_TOPIC], []),
        ("v16_uq_warn_appendix_restates_topic",
         "answer_uq_warn_appendix_restate.md", False, 0, [],
         [linter.W_TOPIC_RESTATED_IN_APPENDIX]),
        ("v16_uq_pass_appendix_reference",
         "answer_uq_pass_appendix_reference.md", False, 0, [], []),
        ("v16_gn_fail_generic_proposal",
         "answer_gn_fail_generic_proposal.md", False, 1,
         [linter.E_GENERIC_UI_ADVICE], []),
        ("v16_gn_fail_generic_fix", "answer_gn_fail_generic_fix.md", False, 1,
         [linter.E_GENERIC_UI_ADVICE], []),
        ("v16_gn_fail_unanchored_proposal",
         "answer_gn_fail_unanchored_proposal.md", False, 1,
         [linter.E_PROPOSAL_UNANCHORED], []),
        ("v16_gn_warn_unanchored_fix", "answer_gn_warn_unanchored_fix.md",
         False, 0, [], [linter.W_FIX_UNANCHORED]),
    ]
    for name, answer, no_kb, want_exit, want_codes, want_warnings in v16_cases:
        code, report, _ = run_linter(answer, no_kb_arm=no_kb,
                                     card=FIXTURE_CARD)
        if report is None:
            check(name, False, "linter stdout was not JSON")
    ...
    # CF6 — the count the rubric reads. The compliant fixture has three
    # findings and two typed proposals, and all five are in conditional form;
    # removing one slot must move the count by exactly one.
    _, cf_pass_report, _ = run_linter("answer_cf_pass.md", card=FIXTURE_CARD)
    check("v16_cf6_counts_every_argument_unit",
          cf_pass_report is not None
          and cf_pass_report["conditional_form"] == {
              "findings": {"units": 3, "complete": 3},
              "proposals": {"units": 2, "complete": 2},
              "units": 5, "complete": 5},
          "counted=%s" % (cf_pass_report["conditional_form"]
                          if cf_pass_report else None))
    _, cf_one_short, _ = run_linter("answer_cf_fail_no_condition.md",
                                    card=FIXTURE_CARD)
    check("v16_cf6_drops_by_one_when_a_slot_is_removed",
          cf_one_short is not None
          and cf_one_short["conditional_form"]["complete"] == 4
          and cf_one_short["conditional_form"]["units"] == 5,
          "counted=%s" % (cf_one_short["conditional_form"]
                          if cf_one_short else None))

    # UQ2 — the collision is reported with both sections, so a reader can go
    # and merge the two bullets without re-reading the answer.
    _, dup_report, _ = run_linter("answer_uq_fail_duplicate_topic.md",
                                  card=FIXTURE_CARD)
    check("v16_uq2_names_both_sections_of_a_collision",
          dup_report is not None
          and dup_report["topics"]["duplicates"] == [{
              "slug": "maturity-horizon-retention",
              "sections": ["findings", "non-monetization effects"]}],
          "duplicates=%s" % (dup_report["topics"]["duplicates"]
                             if dup_report else None))

    # GN1 — a proposal is anchored by a number of the CARD, not by a source
    # ID. The unanchored fixture still cites T9-01 and P-90 and still fails.
    _, unanchored, _ = run_linter("answer_gn_fail_unanchored_proposal.md",
                                  card=FIXTURE_CARD)
    check("v16_gn1_source_id_is_not_a_card_anchor",
          unanchored is not None
          and unanchored["generic_screen"]["proposals_checked"] == 2
          and unanchored["generic_screen"]["proposals_anchored"] == 1,
          "screen=%s" % (unanchored["generic_screen"] if unanchored else None))

    # GN1 — without --card the anchor half cannot run, so it must not fail an
    # answer it never checked.
    code_nocard, nocard, _ = run_linter("answer_gn_fail_unanchored_proposal.md")
    check("v16_gn1_without_card_does_not_fail_unchecked_anchors",
          nocard is not None
          and linter.E_PROPOSAL_UNANCHORED not in error_codes(nocard)
          and nocard["generic_screen"]["card_supplied"] is False,
          "codes=%s" % (error_codes(nocard) if nocard else None))

    # --- V1.6 unit tests: the four new parsers ----------------------------
    for label in linter.STATEMENT_LABELS:
        got, reason = linter.opening_statement_label(
            "[%s] the synthetic mechanism holds" % label)
        check("v16_unit_label_" + label, got == label and reason is None,
              "parsed=%s reason=%s" % (got, reason))
    check("v16_unit_label_survives_emphasis",
          linter.opening_statement_label("**[fact]** measured")[0] == "fact")
    check("v16_unit_label_absent",
          linter.opening_statement_label("the mechanism holds")
          == (None, "absent"))
    check("v16_unit_label_multiple_is_two_mechanisms",
          linter.opening_statement_label("[fact] one and [hypothesis] another")
          == (None, "multiple"))
    check("v16_unit_label_must_lead",
          linter.opening_statement_label("the mechanism holds [fact]")
          == (None, "not-leading"))
    check("v16_unit_label_none_for_missing_slot",
          linter.opening_statement_label(None) == (None, "absent"))

    check("v16_unit_topic_reads_a_kebab_slug",
          linter.read_topic("- [topic: guardrail-margin-retention] x")
          == ("guardrail-margin-retention", None))
    check("v16_unit_topic_rejects_one_word",
          linter.read_topic("- [topic: retention] x")[1] == "malformed")
    check("v16_unit_topic_rejects_seven_words",
          linter.read_topic("- [topic: a-b-c-d-e-f-g] x")[1] == "malformed")
    check("v16_unit_topic_rejects_underscores",
          linter.read_topic("- [topic: guardrail_margin] x")[1] == "malformed")
    check("v16_unit_topic_absent",
          linter.read_topic("- a bullet with no tag")[1] == "absent")
    check("v16_unit_topic_two_tags_are_malformed",
          linter.read_topic("- [topic: a-b] [topic: c-d] x")[1] == "malformed")

    for phrase, expect in (
            ("make the entry point more visible", True),
            ("improve the copy on the paywall", True),
            ("add a CTA below the fold", True),
            ("A/B test the colour of the button", True),
            ("reduce friction in the checkout", True),
            ("simplify the flow", True),
            ("move the button behind the first save, keeping the 64% reach",
             False),
            ("increase trial start rate", False),
            ("read the guardrail after the maturity horizon", False)):
        hits = linter.generic_advice_hits(phrase)
        check("v16_unit_generic_%s" % phrase.split()[0].lower().strip("a/"),
              bool(hits) == expect,
              "phrase=%r hits=%s expect=%s" % (phrase, hits, expect))

    card_numbers = [64.0, 25.0, 30.0, 3.0, 1.5, 4.2]
    check("v16_unit_anchor_number_from_card",
          linter.card_anchored("keeps the 64% branch reach", card_numbers))
    check("v16_unit_anchor_computed_label_counts",
          linter.card_anchored("[computed] anything", []))
    check("v16_unit_anchor_scaffolding_does_not_count",
          not linter.card_anchored("100% of the surface", card_numbers))
    check("v16_unit_anchor_absent",
          not linter.card_anchored("make it clearer for everyone",
                                   card_numbers))
    check("v16_unit_anchor_source_id_is_not_an_anchor",
          not linter.card_anchored("grounded in T9-01 and P-90",
                                   card_numbers))

    for word in linter.DIRECTION_WORDS:
        check("v16_unit_direction_" + word.replace(" ", "_"),
              any(r.search("the metric goes %s here" % word)
                  for r in linter.DIRECTION_RES),
              "the closed lexicon must match its own entry %r" % word)
    check("v16_unit_direction_rejects_a_non_direction",
          not any(r.search("the metric responds")
                  for r in linter.DIRECTION_RES))
    check("v16_unit_direction_needs_a_word_boundary",
          not any(r.search("the group upstream")
                  for r in linter.DIRECTION_RES),
          "`up` must not fire inside `upstream`")
