# Combined Validator Prompt V1.6 (frozen 2026-08-08, FLOW-641)

This is the frozen prompt for BOTH arms of the KB V1.6 evaluation
...
V1.6 closes the three FLOW-628 rubric requirements that no earlier version
owned. Rule 14 (KNOWLEDGE CONTEXT §2.10) puts every statement about the case
under review into the form *if X, grounded in Y, then Z*, with Y carrying its
statement label, so the reader can see whether a conclusion stands on a
measurement or on an assumption without leaving the sentence. Rule 15 (§2.11)
makes each MAIN bullet declare the mechanism-and-segment it is about, and
forbids two bullets from declaring the same one. Rule 16 (§2.12) keeps advice
that could have been written without reading this card out of the main answer.
The section list changes in two places: MAIN bullets carry a topic tag and
proposals carry the conditional slots, and the appendix gains
`## E. Generic suggestions`. Rules 1-13, the closeness model, the analog-card
format, the placeholders and the `<knowledge-context>` block are unchanged from
V1.5 — the Phase B runner does not change. Rule 4 is again untouched.

V1.5 differed from V1.4 in the form of the objections and in the section list:
rule 13 requires every finding in the MAIN part to say what the reader gets
instead of an answer, why, and what it costs, in one of five price units; the
findings are ranked by that price and at most three are blocking (§2.9). The
MAIN sections `## Top risks & failure modes`, `## Blocking design fixes` and
`## What this experiment cannot show` were replaced by the single ranked
section `## Findings`, and `## What you decide` split the open decisions
between the product owner and the analyst.
