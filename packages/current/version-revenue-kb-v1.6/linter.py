#!/usr/bin/env python3
"""Deterministic answer linter for Revenue Validator V1.6 (FLOW-641).
...
# --- V1.6 hard-error codes (KB §2.10, conditional form) --------------------
E_FINDING_NO_CONDITION = "E_FINDING_NO_CONDITION"                # CF2
E_GROUNDS_UNLABELLED = "E_GROUNDS_UNLABELLED"                    # CF3
E_PROPOSAL_NO_CONDITIONAL_FORM = "E_PROPOSAL_NO_CONDITIONAL_FORM"  # CF4
E_CONSEQUENT_NO_DIRECTION = "E_CONSEQUENT_NO_DIRECTION"          # CF5

# --- V1.6 hard-error codes (KB §2.11, one idea one place) ------------------
E_TOPIC_MISSING = "E_TOPIC_MISSING"                              # UQ1
E_TOPIC_MALFORMED = "E_TOPIC_MALFORMED"                          # UQ1
E_DUPLICATE_TOPIC = "E_DUPLICATE_TOPIC"                          # UQ2

# --- V1.6 hard-error codes (KB §2.12, no generic advice) -------------------
E_PROPOSAL_UNANCHORED = "E_PROPOSAL_UNANCHORED"                  # GN1
E_GENERIC_UI_ADVICE = "E_GENERIC_UI_ADVICE"                      # GN2

# --- V1.6 warning codes ----------------------------------------------------
W_TOPIC_RESTATED_IN_APPENDIX = "W_TOPIC_RESTATED_IN_APPENDIX"    # UQ3
W_FIX_UNANCHORED = "W_FIX_UNANCHORED"                            # GN1
...
MAX_PROPOSALS = 3
# The V1.6 form codes. They are reported like any other error and they fail the
# answer, but they are not inputs to the §2.7 PP3 grounding decision.
FORM_CODES = frozenset((
    "E_PROPOSAL_NO_CONDITIONAL_FORM", "E_GROUNDS_UNLABELLED",
    "E_CONSEQUENT_NO_DIRECTION", "E_PROPOSAL_UNANCHORED",
    "E_GENERIC_UI_ADVICE", "E_TOPIC_MISSING", "E_TOPIC_MALFORMED",
    "E_DUPLICATE_TOPIC",
))
...
def check_product_proposals(answer_text, known_ids, no_kb_arm, cards):
    ...
    for index, bullet in enumerate(bullets, start=1):
        ...
        # CF4/CF5 — a typed proposal is written as if X, grounded in Y,
        # then Z. Checked in both arms and before the grounding rules of PP3:
        # the form is what the reader parses, whether or not the KB is there.
        report["typed_count"] += 1
        slots = find_slots(bullet)
        missing = [name for name in ("if", "grounds", "then")
                   if slots.get(name) is None]
        if missing:
            entry["errors"].append({
                "code": E_PROPOSAL_NO_CONDITIONAL_FORM,
                "detail": "[%s] proposal has no %s slot; a proposal is written "
                          "as If: / Grounds: / Then: (KB §2.10 CF4)"
                          % (kind, " and no ".join("'%s:'" % n.capitalize()
                                                   for n in missing)),
            })
        label, reason = opening_statement_label(slots.get("grounds"))
        entry["grounds_label"] = label
        if slots.get("grounds") is not None and label is None:
            entry["errors"].append({
                "code": E_GROUNDS_UNLABELLED,
                "detail": "[%s] proposal has %s statement label on its "
                          "'Grounds:' slot; it opens with exactly one of "
                          "[fact] / [interpretation] / [hypothesis] / "
                          "[computed] (KB §2.10 CF3)"
                          % (kind, {"absent": "no",
                                    "multiple": "more than one",
                                    "not-leading": "a non-leading"}[reason]),
            })
        then_slot = slots.get("then")
        if then_slot is not None and not any(r.search(then_slot)
                                             for r in DIRECTION_RES):
            entry["errors"].append({
                "code": E_CONSEQUENT_NO_DIRECTION,
                "detail": "[%s] proposal states no direction in its 'Then:' "
                          "slot; use one word of the closed lexicon of "
                          "KB §2.10 CF5" % kind,
            })
        if not missing and label is not None and then_slot is not None \
                and any(r.search(then_slot) for r in DIRECTION_RES):
            entry["conditional"] = True
            report["conditional_complete"] += 1
        ...
        # §2.7 PP3 decides grounding; the form rules of §2.10/§2.12 report
        # separately and must not silently turn a grounded proposal into an
        # ungrounded one — that would move the V1.3 verdict for a V1.6 reason.
        pp_errors = [e for e in entry["errors"]
                     if e["code"] not in FORM_CODES]
        if qualifying and not pp_errors:
            entry["grounded"] = True
...
# V1.6 (KB §2.10): `if` opens a finding, `grounds` and `then` carry a
# proposal. The lookbehind keeps `if:` inside a word — or a possessive — from
# being read as a slot marker.
SLOT_NAMES = ("if", "mechanism", "consequence", "price", "fix",
              "grounds", "then")
SLOT_RE = re.compile(
    r"(?<![A-Za-z])[*_`]{0,2}\s*"
    r"(if|mechanism|consequence|price|fix|grounds|then)"
    r"\s*[*_`]{0,2}\s*:",
    re.IGNORECASE,
)

# CF3 — the four statement labels of rule 8. The grounds slot OPENS with
# exactly one of them, which is what tells a measurement from an assumption.
STATEMENT_LABELS = ("fact", "interpretation", "hypothesis", "computed")
STATEMENT_LABEL_RE = re.compile(
    r"\[\s*(%s)\s*\]" % "|".join(STATEMENT_LABELS), re.IGNORECASE
)

# CF5 — the closed direction lexicon. A direction is not a magnitude: rule 4
# is untouched by this check.
DIRECTION_WORDS = (
    "up", "down", "higher", "lower", "rise", "rises", "fall", "falls",
    "grow", "grows", "shrink", "shrinks", "increase", "increases",
    "decrease", "decreases", "improve", "improves", "worsen", "worsens",
    "more", "less", "fewer", "unchanged", "flat", "no change",
)
DIRECTION_RES = tuple(
    re.compile(r"(?<!\w)" + re.escape(w).replace(r"\ ", r"\s+") + r"(?!\w)",
               re.IGNORECASE)
    for w in DIRECTION_WORDS
)

# UQ1 — the topic tag. The slug is read raw so a malformed one can be named
# in the error instead of silently counting as absent.
TOPIC_TAG_RE = re.compile(r"\[\s*topic\s*:\s*([^\]]*)\]", re.IGNORECASE)
TOPIC_SLUG_RE = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)+$")
TOPIC_MIN_WORDS = 2
TOPIC_MAX_WORDS = 6
# UQ3 — an appendix mention is a pointer when `see` immediately precedes it.
TOPIC_REFERENCE_RE = re.compile(
    r"\bsee\s*:?\s*$", re.IGNORECASE
)

# GN2 — the closed lexicon of KB §2.12: formulations that name a cosmetic
# change and carry no mechanism through the money chain. Matched against
# normalised text (emphasis stripped, whitespace collapsed, lower-cased).
GENERIC_ADVICE_PATTERNS = (
    r"mak\w+\s+(?:\w+\s+){0,3}(?:more\s+)?(?:visible|prominent|noticeable|"
    r"obvious|salient|clearer|discoverable)",
    r"(?:bigger|larger|brighter|bolder|higher[- ]contrast)\s+"
    r"(?:button|cta|banner|text|font|label)",
    r"improv\w+\s+the\s+(?:copy|wording|ux|ui|design|onboarding|"
    r"experience|messaging)",
    r"add(?:ing)?\s+an?\s+(?:cta|call[- ]to[- ]action|button|banner|tooltip|"
    r"badge|popup|pop-up)",
    r"a\s*/?\s*b[- ]test\s+the\s+(?:colou?r|copy|button|placement|wording)",
    r"optimi[sz]\w+\s+the\s+(?:funnel|flow|onboarding|paywall|ui|ux)",
    r"increas\w+\s+(?:visibility|awareness|engagement)(?!\s*\w*\s*rate)",
    r"better\s+(?:ux|ui|copy|design|placement|messaging|wording)",
    r"simplif\w+\s+the\s+(?:flow|ui|interface|form)",
    r"reduc\w+\s+friction",
    r"more\s+prominent\s+placement",
    r"clearer\s+(?:copy|wording|messaging|value\s+prop\w*)",
)
GENERIC_ADVICE_RES = tuple(
    re.compile(p, re.IGNORECASE) for p in GENERIC_ADVICE_PATTERNS
)
...
def opening_statement_label(slot_text):
    """CF3 — the single rule-8 label a grounds slot opens with, or a reason.

    Returns `(label, None)` when the slot opens with exactly one label, and
    `(None, reason)` otherwise. `reason` is one of `"absent"`, `"multiple"`,
    `"not-leading"` — the three ways a reader is left unable to tell a
    measurement from an assumption.
    """
    if slot_text is None:
        return None, "absent"
    stripped = strip_emphasis(slot_text).strip()
    found = STATEMENT_LABEL_RE.findall(stripped)
    if not found:
        return None, "absent"
    if len(found) > 1:
        return None, "multiple"
    lead = STATEMENT_LABEL_RE.match(stripped)
    if lead is None:
        return None, "not-leading"
    return lead.group(1).lower(), None


def normalize_prose(text):
    """Lower-cased, emphasis-free, whitespace-collapsed copy for lexicon scans."""
    return re.sub(r"\s+", " ", strip_emphasis(text)).strip().lower()


def generic_advice_hits(text):
    """GN2 — every phrase of the KB §2.12 lexicon this text matches."""
    scan = normalize_prose(text)
    hits = []
    for pattern in GENERIC_ADVICE_RES:
...
def card_anchored(text, card_numbers):
    """GN1 — does this text carry a number the experiment card states?

    A `[computed]` statement counts on its own: §2.8 already proved every
    number inside it comes from the card. Bare 0, 1 and 100 do not count —
    they are arithmetic scaffolding (KB §2.8 CC1), not evidence that the
    writer opened the document.
    """
    if COMPUTED_LABEL_RE.search(text):
        return True
    cleaned = SOURCE_ID_RE.sub(" ", normalize_math(text))
    for raw in PROSE_NUMBER_RE.findall(cleaned):
        try:
            ...
            continue
        if abs(value) in FREE_CONSTANTS:
            continue
        if value_matches(value, card_numbers, decimals_of(raw)):
            return True
    return False


def read_topic(bullet_text):
    """UQ1 — `(slug, reason)` for a bullet's topic tag.

    `reason` is `"absent"` when no tag is present and `"malformed"` when the
    tag is there but the slug is not kebab-case of two to six words.
    """
    tags = TOPIC_TAG_RE.findall(bullet_text)
    if not tags:
        return None, "absent"
    if len(tags) > 1:
        return None, "malformed"
    raw = strip_emphasis(tags[0]).strip().lower()
    if not TOPIC_SLUG_RE.match(raw):
        return None, "malformed"
    words = raw.split("-")
...
def check_findings(answer_text, known_ids, card_text):
    ...
    for index, bullet in enumerate(bullets, start=1):
        ...
        hits = generic_advice_hits(headline)
        if hits:
            entry["generic_hits"] = hits
            entry["errors"].append({
                "code": E_GENERIC_UI_ADVICE,
                "detail": "finding %d states its headline as a cosmetic change "
                          "(%s); it carries no mechanism through the money "
                          "chain (KB §2.12 GN2)" % (index, ", ".join(hits)),
            })

        slots = find_slots(bullet)

        # CF2 — the condition comes first, before the mechanism.
        marks = {m.group(1).lower(): m.start()
                 for m in reversed(list(SLOT_RE.finditer(bullet)))}
        if "if" not in slots:
            entry["errors"].append({
                "code": E_FINDING_NO_CONDITION,
                "detail": "finding %d has no 'If:' slot, so it does not say "
                          "under which condition it bites (KB §2.10 CF2)"
                          % index,
            })
        elif "mechanism" in marks and marks["if"] > marks["mechanism"]:
            entry["errors"].append({
                "code": E_FINDING_NO_CONDITION,
                "detail": "finding %d carries 'If:' after 'Mechanism:'; the "
                          "condition is the first slot (KB §2.10 CF2)" % index,
            })
        else:
        ...
        if mechanism is None:
            ...
            # CF3 — grounded is not enough: the reader must see WHAT KIND of
            # ground it is, or a measured number and an assumed one read alike.
            label, reason = opening_statement_label(mechanism)
            entry["grounds_label"] = label
            if label is None:
                entry["errors"].append({
                    "code": E_GROUNDS_UNLABELLED,
                    "detail": "finding %d has %s statement label on its "
                              "'Mechanism:' slot; it opens with exactly one of "
                              "[fact] / [interpretation] / [hypothesis] / "
                              "[computed] (KB §2.10 CF3)"
                              % (index, {"absent": "no",
                                         "multiple": "more than one",
        ...
        # CF6 — a finding is in conditional form when all three parts are
        # visible: the condition, the labelled grounds, the consequence.
        if (entry["condition"] and entry["grounds_label"]
                and slots.get("consequence") is not None):
            report["conditional_complete"] += 1
...
# ---------------------------------------------------------------------------
# V1.6: one idea, one place (KB §2.11) and the generic-advice screen (§2.12)
# ---------------------------------------------------------------------------

# UQ1 — the three MAIN sections whose bullets are statements about this case.
# `## What you decide` is deliberately absent: a decision is a question put to
# a named role, not a statement about a mechanism.
TOPIC_SECTIONS = (
    ("findings", "FINDINGS_HEAD_RE"),
    ("product proposals", "PRODUCT_PROPOSALS_HEAD_RE"),
    ("non-monetization effects", "SIDE_EFFECTS_HEAD_RE"),
)


def extract_appendix(answer_text):
    """Everything from the `# APPENDIX` banner to the end, or "" if absent."""
    lines = answer_text.splitlines()
    for i, line in enumerate(lines):
        if APPENDIX_HEAD_RE.match(line):
            return "\n".join(lines[i + 1:])
    return ""


def check_topics(answer_text):
    """KB §2.11 UQ1-UQ3. Returns (report_dict, errors, warnings)."""
    errors = []
    warnings = []
    report = {"tags": [], "duplicates": [], "restated_in_appendix": []}

    main = extract_main(answer_text)
    seen = {}
    for name, head_re in ((n, globals()[r]) for n, r in TOPIC_SECTIONS):
        section = extract_section(main, head_re)
        if section is None:
            continue
        for index, bullet in enumerate(split_bullets(section), start=1):
            slug, reason = read_topic(bullet)
            entry = {"section": name, "index": index, "slug": slug}
            report["tags"].append(entry)
            if slug is None:
                code = E_TOPIC_MISSING if reason == "absent" else E_TOPIC_MALFORMED
                errors.append({
                    "code": code,
                    "detail": "bullet %d of '%s' carries %s topic tag; the "
                              "syntax is [topic: <slug>] with a kebab-case "
                              "slug of %d to %d words naming the mechanism and "
                              "the segment (KB §2.11 UQ1)"
                              % (index, name,
                                 "no" if reason == "absent" else "a malformed",
                                 TOPIC_MIN_WORDS, TOPIC_MAX_WORDS),
                })
                continue
            if slug in seen:
                first = seen[slug]
                ...
                errors.append({
                    "code": E_DUPLICATE_TOPIC,
                    "detail": "topic %r appears in '%s' and again in '%s'; two "
                              "statements about one mechanism on one segment "
                              "are one statement (KB §2.11 UQ2)"
                              % (slug, first["section"], name),
                })
            else:
                seen[slug] = entry

    # UQ3 — the appendix points at a MAIN topic, it does not restate it.
    appendix = extract_appendix(answer_text)
    if appendix and seen:
        for m in TOPIC_TAG_RE.finditer(appendix):
            raw = strip_emphasis(m.group(1)).strip().lower()
            if raw not in seen:
                continue
            before = appendix[max(0, m.start() - 12):m.start()]
            if TOPIC_REFERENCE_RE.search(before):
                continue
            if raw in report["restated_in_appendix"]:
                continue
            report["restated_in_appendix"].append(raw)
            warnings.append({
                "code": W_TOPIC_RESTATED_IN_APPENDIX,
                "detail": "the appendix carries topic %r again without "
                          "pointing at the MAIN bullet ('see [topic: %s]') "
                          "(KB §2.11 UQ3)" % (raw, raw),
            })
    return report, errors, warnings


def check_generic(answer_text, card_text):
    """KB §2.12 GN1-GN2. Returns (report_dict, errors, warnings)."""
    errors = []
    warnings = []
    report = {
        "card_supplied": card_text is not None,
        "proposals_checked": 0,
        "proposals_anchored": 0,
        "fixes_checked": 0,
        "fixes_anchored": 0,
        "flagged": [],
    }
    ...
    # GN1/GN2 on the typed proposals. `[ungrounded]` bullets are exempt: they
    # assert nothing about this case, they name a research gap.
    section = extract_section(main, PRODUCT_PROPOSALS_HEAD_RE)
    if section is not None:
        for index, bullet in enumerate(split_bullets(section), start=1):
            first_line = bullet.splitlines()[0]
            types = [t.lower() for t in PROPOSAL_TYPE_RE.findall(first_line)]
            if len(types) != 1 or types[0] == "ungrounded":
                continue
            report["proposals_checked"] += 1
            hits = generic_advice_hits(bullet)
            if hits:
                report["flagged"].append({
                    "where": "product proposal %d" % index, "phrases": hits,
                })
                errors.append({
                    "code": E_GENERIC_UI_ADVICE,
                    "detail": "product proposal %d is a cosmetic change (%s) "
                              "with no mechanism through the money chain "
                              "(KB §2.12 GN2)" % (index, ", ".join(hits)),
                })
            if card_text is None:
                continue
            if card_anchored(bullet, card_numbers):
                report["proposals_anchored"] += 1
            else:
                errors.append({
                    "code": E_PROPOSAL_UNANCHORED,
                    "detail": "product proposal %d carries no number this card "
                              "states, so it would read the same for any other "
                              "experiment; a source ID is not an anchor for "
                              "this rule (KB §2.12 GN1)" % index,
                })

    # GN2 on the `Fix:` slots, and GN1 on them as a warning: a fix is an
    # instruction, and an unanchored instruction is the same defect one
    # notch weaker than an unanchored proposal.
    findings = extract_section(main, FINDINGS_HEAD_RE)
    if findings is not None:
        for index, bullet in enumerate(split_bullets(findings), start=1):
            fix = find_slots(bullet).get("fix")
            if fix is None:
                continue
            report["fixes_checked"] += 1
            hits = generic_advice_hits(fix)
            if hits:
                report["flagged"].append({
                    "where": "fix of finding %d" % index, "phrases": hits,
                })
                errors.append({
                    "code": E_GENERIC_UI_ADVICE,
                    "detail": "the fix of finding %d is a cosmetic change (%s) "
                              "with no mechanism through the money chain "
                              "(KB §2.12 GN2)" % (index, ", ".join(hits)),
                })
            if card_text is None:
                continue
            if card_anchored(bullet, card_numbers):
                report["fixes_anchored"] += 1
            else:
                warnings.append({
                    "code": W_FIX_UNANCHORED,
                    "detail": "the fix of finding %d names no number of this "
                              "card (KB §2.12 GN1)" % index,
                })
    return report, errors, warnings
...
def lint(answer_text, known_ids, no_kb_arm, gen_classes=(), card_text=None,
    ...
    # V1.6 (KB §2.11): one idea, one place. Applied in both arms — it reads the
    # answer's own topic tags and needs neither a KB nor the card.
    topics, topic_errors, topic_warnings = check_topics(answer_text)
    report["topics"] = topics
    report["errors"].extend(topic_errors)
    report["warnings"].extend(topic_warnings)

    # V1.6 (KB §2.12): the generic-advice screen. The card anchor needs
    # `--card`; without it only the lexicon half runs.
    generic, generic_errors, generic_warnings = check_generic(
        answer_text, card_text
    )
    report["generic_screen"] = generic
    report["errors"].extend(generic_errors)
    report["warnings"].extend(generic_warnings)

    # V1.6 (KB §2.10 CF6): how many MAIN argument units are in conditional
    # form. A count, not a verdict — the errors above carry the verdict.
    finding_units = len(findings.get("bullets", []))
    proposal_units = proposals.get("typed_count", 0)
    report["conditional_form"] = {
        "findings": {
            "units": finding_units,
            "complete": findings.get("conditional_complete", 0),
        },
        "proposals": {
            "units": proposal_units,
            "complete": proposals.get("conditional_complete", 0),
        },
        "units": finding_units + proposal_units,
        "complete": (findings.get("conditional_complete", 0)
                     + proposals.get("conditional_complete", 0)),
    }
