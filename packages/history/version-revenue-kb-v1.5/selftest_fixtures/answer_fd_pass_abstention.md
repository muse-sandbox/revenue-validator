...
- **[stop]** We spend the slot and get a result nobody can act on: no computable limit. *Mechanism:* the document states no reach, no expected effect and no detectable difference, so the coverage share that would drive the arithmetic is the number that is missing. *Consequence:* the reader cannot tell whether this design could ever resolve the effect it is built for. *Price:* decision impossible — the limit of the design is unknown before launch.
