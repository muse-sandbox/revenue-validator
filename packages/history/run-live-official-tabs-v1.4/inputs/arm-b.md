...
- `## What this experiment cannot show` — MANDATORY in every answer, with or
  without KNOWLEDGE CONTEXT. Per rule 8 / §2.8: take the numbers the card
  states about itself — reach, coverage, expected effect, guardrail margin,
  detectable difference — combine them, and say what this design will
  therefore fail to resolve. At least one `[computed]` statement, with the
  arithmetic written out in the sentence and no source ID. If the card does
  not state enough numbers to compute a limit, write the exact literal
  `no computable limit` and name the number that is missing. This belongs
  here, in MAIN, and not in the methodology appendix: it is an argument
  about whether to run the experiment at all.
...
`## Non-monetization effects to instrument` section, the
`## What this experiment cannot show` section, and honesty about the missing
evidence base remain mandatory. The last of these is unaffected by the
absence of a KNOWLEDGE CONTEXT: its numbers come from the card, and an arm
without an evidence base has exactly the same access to them.

### KNOWLEDGE CONTEXT (present only in arm B)

<knowledge-context>
# Revenue Knowledge Base V1.4 — Corpus & Closeness Model (FLOW-632, frozen 2026-08-07)
...
2. Closeness model (L1/L2/L3): deterministic level computation, the strict
   ...
   proposals (§2.7), and the provenance rules for computed claims (§2.8)
...
|---|---|---|
| risk | what could break this experiment or this idea | `Top risks & failure modes` |
...
**PP5 — Do not restate a risk as a proposal.** The risk bullet names the
failure mode; the proposal bullet names the alternative. Where one source
feeds both, the two bullets must say different things. A grounding source
cited in the risks section and nowhere in the proposals is not an error, but
it is usually a missed proposal — the linter reports it as a warning, not a
failure.
...
**CC5 — The MAIN slot is mandatory.** Every answer carries the heading
`## What this experiment cannot show` in the MAIN part (an ordinal prefix
such as `## 6. What this experiment cannot show` is allowed), and the
section contains at least one `[computed]` statement. This is where the
arithmetic goes: in the two runs that motivated this section, the same
calculation was attempted inside the methodology appendix, where nobody
reads it as an argument about the idea.

**CC6 — Honest abstention (hard).** When the card genuinely does not state
enough numbers to compute a limit, the section must contain the exact
literal `no computable limit` and one sentence naming the number that is
missing. Inventing a plausible figure to fill the slot is the failure this
