#!/usr/bin/env python3
"""Assemble the arm-A / arm-B inference inputs for the V1.4 trial run.
...
the V1.3 runs used — so that the only difference between the V1.3 and V1.4
runs is the validator bundle.

Besides the two arm inputs this writes `inputs/experiment_card.md`: the card
alone, byte-identical to the copy embedded in both arm inputs. The V1.4
...
BUNDLE = os.path.normpath(os.path.join(HERE, "..", "revenue-kb-v1.4"))
...
def prompt_body():
    ...
    text = read(os.path.join(BUNDLE, "validator_prompt_v1_4.md"))
    marker = "## PROMPT\n"
